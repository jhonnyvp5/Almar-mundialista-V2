...
export default function App() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (data) {
        ...
        // Check for new app version to automatically log out active sessions and refresh assets
        if (data.appVersion) {
          const storedVersion = localStorage.getItem('local_app_version');
          if (storedVersion && storedVersion !== data.appVersion) {
            localStorage.setItem('local_app_version', data.appVersion);
            localStorage.removeItem('currentUser');
            alert('🔄 Actualización Detectada: Se han aplicado mejoras en el sistema. Tu sesión se ha cerrado para mostrarte la nueva versión optimizada.');
  ...
  const handleSaveGroupSelections = async (gId: string, firstVal: string, secondVal: string, thirdVal: string, silent = false) => {
    if (!currentUser) return;
    if (systemConfig?.maintenance_mode && currentUser?.role !== 'admin') {
      if (!silent) showToast('🔒 El sistema está en mantenimiento. No se permiten registrar o modificar pronósticos.');
      return;
    }
    if (isPastDeadline()) {
  ...
  const isGroupStageSelectionsLocked = (): boolean => {
    ...
    const serverTimeMs = Date.now();
    // First match starts on 2026-06-11 at 14:00 ECU (UTC-5)
    const firstMatchTimeMs = new Date('2026-06-11T14:00:00-05:00').getTime();
              ...
              <div className="lg:col-span-4 bg-slate-900/40 p-5 rounded-2xl border border-slate-900 flex flex-col justify-between space-y-4">
                ...
                {adminStats ? (
                  <div className="grid grid-cols-2 gap-4 flex-1 pt-2">
                    ...
                    <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-855 space-y-2 flex flex-col justify-center min-h-[90px] col-span-2 sm:col-span-1">
                      ...
                      {adminStats.breakdown && (
                        <div className="pt-2 border-t border-slate-850 grid grid-cols-3 gap-1 text-[9px] text-slate-400 font-medium">
                          <div className="space-y-0.5">
                            <div className="text-[8px] text-slate-500 font-extrabold uppercase tracking-widest">F. Grupos</div>
                            <div className="font-mono text-white font-bold">{adminStats.breakdown.groupStandings} <span className="text-slate-500">sel.</span></div>
                          </div>
                          <div className="space-y-0.5 border-l border-slate-850 pl-2">
                            <div className="text-[8px] text-slate-500 font-extrabold uppercase tracking-widest">Partidos</div>
                            <div className="font-mono text-white font-bold">{adminStats.breakdown.matchesAndKnockouts} <span className="text-slate-500">pron.</span></div>
                          </div>
                          <div className="space-y-0.5 border-l border-slate-850 pl-2">
                            <div className="text-[8px] text-slate-500 font-extrabold uppercase tracking-widest">P. FIFA</div>
                            <div className="font-mono text-white font-bold">{adminStats.breakdown.fifaAwards} <span className="text-slate-500">votos</span></div>
                          </div>
                        </div>
                      )}
                    </div>
